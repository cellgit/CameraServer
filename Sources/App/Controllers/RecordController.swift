...
class RecordController: RouteCollection {
    ...
    func boot(router: Router) throws {
        ...
        router.group("record") { (group) in
            ...
            // post 获取动态
            group.post(PageContainer.self, at: "getRecords", use: postAllRecordsHandler)
...
extension RecordController {
    ...
    /// post 获取动态
    private func postAllRecordsHandler(_ req: Request,container: PageContainer) throws -> Future<Response> {
        
        let futureAll = Record.query(on: req).query(page: req.page).all()
